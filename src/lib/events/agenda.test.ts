...
const conf = { status: 'confirmado', deleted_at: undefined, google_event_id: 'g1', google_event_link: 'https://calendar.google.com/x', google_public_event_id: 'p1' } as const;
...
describe('leituraDaAgenda', () => {
  it('confirmado e no Google: sincronizado, com link e se é público', () => {
    const l = leituraDaAgenda(conf, aviso('enviado'));
    expect(l).toMatchObject({ estado: 'sincronizado', publico: true, link: 'https://calendar.google.com/x', quando: '2026-09-15T14:22:00Z' });
    expect(leituraDaAgenda({ ...conf, google_public_event_id: undefined }, aviso('enviado'))?.publico).toBe(false);
  });
  ...
  it('pendente sem nada no Google e sem aviso: nada a mostrar', () => {
    expect(leituraDaAgenda({ status: 'pendente', deleted_at: undefined, google_event_id: undefined, google_event_link: undefined, google_public_event_id: undefined }, null)).toBeNull();
